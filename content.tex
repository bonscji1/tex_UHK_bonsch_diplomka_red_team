...
recent years, mostly because of the constant demand from criminal organisations for easy access to organisation
...
their e-mail appear much more legitimate. Other ways include compromising a Microsoft Exchange server via
ProxyShell or ProxyLogin and then distributing phishing e-mails to internal and external user accounts. To
...
Phishing se snaží o krádež důležitých informací, jako jsou čísla kreditních karet a hesla, prostřednictvím e-mailů využívajících sociálního inženýrství a klamání. Spear-phishing je sofistikovanější verze phishingu, která cílí na konkrétní organizace nebo jednotlivce. Whaling je spear-phishingový útok zaměřený na uživatele ve vysokých pozicích (vedoucí pracovníky, politiky atd.). Smishing, termín odvozený z kombinace slov 'SMS' a 'phishing', nastává, když jsou finanční nebo osobní informace obětí získávány prostřednictvím SMS zpráv. Další související hrozbou je vishing, což je kombinace slov 'phishing' a 'voice', který nastává, když jsou informace poskytovány prostřednictvím telefonu, kde zlomyslní aktéři využívají sociálně-inženýrských technik k získání citlivých informací od uživatelů.
...
Vytváření phishingových webových stránek a nastavení základní infrastruktury pro kampaň sociálního inženýrství může být nudnou prací. Kriminálníci se stále častěji obrací na hotový materiál nabízený phishingovými sadami nebo využívají služební model nazývaný 'Phishing-as-a-Service'.

Nasazení phishingových sad obvykle má krátkou životnost, přičemž téměř třetina nasazených sad se použije ne déle než jeden den.
Podle společnosti Microsoft jsou moderní phishingové sady dostatečně sofistikované na to, aby se vydávaly za legitimní obsah s použitím správného pravopisu, gramatiky a obrazových prvků.
Ve stejné zprávě Microsoft poznamenává, že i pachatelé, kteří tyto sady používají, mohou být klamáni.
Některé sady obsahují "dodatečnou" funkčnost, která nejenže odesílá získané přihlašovací údaje phisherům, ale také původnímu autorovi sady nebo sofistikovanému zprostředkovateli.
Z pohledu oběti může mít tato skutečnost vážné důsledky, protože odcizené přihlašovací údaje se dostanou do rukou několika různým zločineckým skupinám.
Phishingové sady také zohledňují regionální rozdíly (s použitím blokování na základě geografické polohy), filtrování nežádoucích agentů, možnosti zamlžení a jsou prodávány jako součást balíčku software-as-a-service: Phishing-as-a-Service (PhaaS).
Tyto služby nejsou nové, ale zpráva od společnosti Microsoft o operaci BulletProofLink ukazuje její úroveň sofistikace, profesionální obchodní model a využití automatizace.
...
deployments of phishing kits generally have a short lifespan, with almost one-third of deployed kits
being used for no longer than a day.
According to Microsoft, modern phishing kits are sufficiently sophisticated to
masquerade as legitimate content in their use of spelling, grammar and imagery.
In the same report Microsoft notes
that the miscreants making use of these kits can also be fooled.
Some of the kits contain 'added' functionality that not
...
obfuscation options and are sold as part of a software-as-a-service package: Phishing-as-a-Service (PhaaS).
These services are not new but a report from Microsoft on the BulletProofLink operation show its level of
sophistication, professional business model and its use of automation.

%trend
Kampaň, kterou v březnu 2022 upozornila společnost Proofpoint397, se týká pravděpodobné phishingové kampaně sponzorované státem, která použila možná ohrožený e-mailový účet člena ukrajinských ozbrojených sil k cílení na zaměstnance evropských vlád, kteří se zabývají logistikou uprchlíků prchajících z Ukrajiny. V rámci kampaně byl použit e-mail s předmětem odkazujícím na rozhodnutí Rady bezpečnosti Ukrajiny.
Tým Google TAG pozoroval neustále rostoucí počet aktérů hrozeb, kteří využívají válku na Ukrajině jako návnadu v phishingových a malware kampaních. Jedna z kampaní zahrnovala skupinu COLDRIVER, která je údajně ruským aktérem hrozeb někdy nazývaným také Callisto, který používal phishingové e-maily na získávání přístupových údajů od vládních a obranných úředníků, politiků, nevládních organizací, think tanků a novinářů39.
...
officials, politicians, NGOs, think tanks and journalists39


Phishing útoky jsou prováděny ve velkém objemu a cílí na širokou veřejnost, zatímco jiné útoky sociálního inženýrství využívají vlastní kampaně přizpůsobené konkrétním zaměstnancům.
Při použití sociálního inženýrství útočníci využívají přístup zaměstnanců uvnitř organizace k získání technického základu v síti, ze které provádějí další útoky.


\subsubsection{Vishing}
Forma sociálního inženýrsvtý zaměřená na podvodné telefonáty se nazývá vishing.
Oproti phishingu však klade mnohem větší důraz na improvizaci a komunikační schopnosti útočníka.
hlavní změnou oproti phishingu je silná zpětná vazba.
Útočník je tak schopen měnit plán útoku pomocí nově zjištěných informací, pokud však útočník není dostatečně schopný nebo nemá dostatečně propracovanou zámiku, je snadné ho odhalit.

%todo refine
Podle Europolu se bezpečnostní podvod s bankovním účtem stává novým způsobem provádění vishingu.
V rámci tohoto podvodu útočníci přesvědčují oběti, aby převedly peníze na "bezpečný účet", tím, že jim řeknou, že jejich bankovní účet byl napaden.
Aby byl příběh více přesvědčivý, často se vydávají za policejního důstojníka nebo zaměstnance finanční instituce.
Bohužel, tento takzvaný bezpečný účet je pod kontrolou podvodníků a po dokončení převodů peněz zjistí oběti, že za pár minut ztratily celoživotní úspory.
Objem případů vishingu se rovněž značně zvýšil, jak uvádějí Agari a PhishLabs, a to o ne méně než 550\% za posledních dvanáct měsíců (1. čtvrtletí 2022 ve srovnání s 1. čtvrtletím 2021).
Je velmi pravděpodobné, že budeme i nadále pozorovat tento trend i v blízké budoucnosti.
...
mezi phsihingem a vishingem, bere části od obou, rychlé ale psaný text

%todo refine
Jedním z mobilních bankovních malwarů, který byl pozorován s vysokou intenzitou, je FluBot. Cílí na uživatele zařízení s operačním systémem Android většiny Evropy a šíří se prostřednictvím SMS a MMS zpráv. Oběti nejprve obdrží SMS zprávu (nazývanou smishing nebo SMS phishing), která se vydává za doručovatelské společnosti, hlasové záznamy nebo falešný software. Zpráva obsahuje odkaz, který směřuje na webovou stránku, která instruuje oběť, aby nainstalovala aplikaci. Jakmile je aplikace nainstalována, udělují se požadovaná oprávnění nebo jsou někdy deaktivovány bezpečnostní funkce. FluBot se šíří samošířením tím, že odesílá phishingové textové zprávy z nakaženého zařízení do kontaktů na seznamu. Bude také sdílet tento seznam kontaktů s provozovateli kampaní, ale nejproblémovější pro oběti je, že sbírá čísla kreditních karet a přihlašovací údaje do online bankovnictví, zachytává SMS zprávy (jako jednorázová hesla) a zaznamenává snímky obrazovky.
...
Nejčastěji takto útočník získává výpis celé databáze.
