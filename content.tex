...
Toto rozdělení dává smysl, stejná technika bude jinak aplikována na různých systémech a díky tomuto rozdělení je možné zameřit se právě na jeden určitý use-case útoku.
...
Při identifikaci metodologie útoku na určitou skupinu je možné předpovídat další kroky z předchozích záznamů a zmírnit tak dopad útoku.


\paragraph{Software}
Software je obecný termín pro vlastní nebo komerční kód, utility operačního systému, open source software nebo jiné nástroje používané k provádění chování modelovaného v ATT&CK. Některé instance software mají více jmen spojených se stejnou instancí kvůli různým organizacím, které sledují stejnou sadu software různými názvy. Tým se snaží sledovat překrývání jmen na základě veřejně oznámených asociací a označuje je jako "Associated Software" na každé stránce (dříve označováno jako "Aliases"), protože věříme, že tyto překryvy jsou užitečné pro povědomí analytiků.

Záznamy o software zahrnují veřejně oznámené použití techniky nebo schopnost používat techniku a mohou být mapovány na skupiny, které byly hlášeny, že používají daný software. Poskytnuté informace nezastupují veškeré možné použití techniky pro daný software, ale pouze podmnožinu, která je k dispozici výhradně prostřednictvím zpráv z otevřených zdrojů.

Nástroj - komerční, open-source, vestavěný nebo veřejně dostupný software, který by mohl být použit obráncem, pen testerem, red teamerem nebo útočníkem. Tato kategorie zahrnuje jak software, který se obvykle nenachází na podnikovém systému, tak software obvykle k dispozici jako součást operačního systému, který již existuje v prostředí. Příklady zahrnují PsExec, Metasploit, Mimikatz, stejně jako Windows utilities, jako jsou Net, netstat, Tasklist atd.
Malware - komerční, vlastní uzavřený nebo open-source software určený k použití pro zločinné účely útočníky. Příklady zahrnují PlugX, CHOPSTICK, atd.
\paragraph{Campaigns}

Bezpečnostní komunita sleduje intruzivní aktivity pomocí různých analytických metod a pojmů, jako jsou operace, soubory intruzí a kampaně. Některé intruzivní aktivity mohou být označovány různými jmény kvůli tomu, že různé organizace sledují podobnou aktivitu, často z různých úhlů pohledu. Naopak někdy se o nahlášené aktivitě vůbec neuvádí název.

Závadná kybernetická aktivita může být připsána hrozbě ze strany skupiny, nebo se odkazuje jako nezpochybněná aktivita. Alternativně mohou být složité kybernetické operace zapojeny několik přidružených nebo nepřidružených skupin, přičemž každá hraje jedinečnou roli (tj. počáteční přístup, exfiltrace dat atd.).

Pro účely stránky Kampaně tým MITRE ATT&CK používá pojem kampaň k popisu jakéhokoli seskupení intruzivní aktivity, která proběhla během určitého období s obecnými cíli a úkoly. Nejmenovaná intruzivní aktivita se označuje jedinečným identifikátorem ATT&CK, jinak tým použije název aktivity uvedený v veřejných zprávách. Pro pojmenované kampaně tým činí nejlepší úsilí o sledování překrývajících se jmen, která jsou označena jako "Přidružené kampaně" na každé stránce, protože věříme, že tyto překrývající se názvy jsou pro analytiky užitečné. Záznamy kampaní budou také přiřazeny k stránkám ATT&CK Group a Software, pokud je to možné na základě veřejného hlášení; nezpochybněná aktivita bude jednoduše odkazovat na "hrozbu pro aktéry" v procedurálním příkladu.

Kampaně jsou mapovány na veřejně uváděné techniky a jsou v nich zahrnuty původní reference. Poskytnuté informace nezastupují veškeré možné techniky používané v kampani, ale spíše jejich podmnožinu, která je dostupná prostřednictvím otevřených zdrojů.
