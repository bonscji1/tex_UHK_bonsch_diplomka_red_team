...
Tato znalostní báze je založená na pozorování v rámci reálného světa, je aktivně udržována a každé dva roky podléhá aktualizaci\cite{mitre_attack_framework_faq}.
Hlavním přínosem tohoto projektu však není pouze mapování útočníků, ale celková klasifikace útoků do jednotlivých \textit{Taktik, Technik a Dílčích technik}.

\paragraph{Taktika}

\paragraph{Technika a dílčí technika}

\paragraph{Obrana}

\paragraph{závěrem}
V současné době je MITRE ATT\&CK považován jako jeden z nejvýznamnějších standardů pro popis technik používaných útočníky při kybernetických útocích.
Díky popularitě došlo k jeho velkému růstu, existují proto specifikace pro určitá odvětví:
...
Pomocí této databáze mohou lépe porozumět, jak útočníci pracují a jaké techniky používají, a mohou použít tyto informace k vytvoření lepších obranných strategií.
MITRE ATT\&CK poskytuje informace o různých fázích útoku, včetně získávání informací o cílové organizaci, překonávání zabezpečení a exfiltrace dat.
Každá technika útoku je popsána podrobně včetně kroků, jakými útočníci postupují, aby napadli cílovou organizaci.
