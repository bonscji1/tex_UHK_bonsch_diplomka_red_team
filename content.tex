...
K různým účelům se často používají digitální měny jako paysafecard, Bitcoin a další kryptoměny, což ztěžuje vystopování a trestní stíhání pachatelů.




 Práce se zaměřuje na tři klíčové prvky přítomné v každém ransomware útoku: aktiva, akce a vydírání.
Tato definice je nezávislá na konkrétní akci a byla vytvořena s ohledem na proměnlivou hrozbu ransomware, rozšíření více metod vydírání a různé cíle, než jenom finanční zisk.
Zpráva také pokrývá čtyři základní akce (uzamknout, zašifrovat, smazat a ukrást), které ransomware používá ke způsobení výpadku dostupnosti, důvěrnosti a integrity aktiv.
Může sloužit jako reference k lepšímu porozumění této hrozbě.


Skupiny pokračují v mizení a (znovu)vyvstávání.
Zároveň se často opakovaně využívají nebo kradou jejich zdrojové kódy, což znesnadňuje porozumění tomu, jak jsou tyto skupiny propojeny.
Výzkumníci často vytvářejí předpoklady na základě překryvu zdrojových kódů nebo opakovaně použitých adres peněženek, aby pochopili tuto měnící se krajinu.

Útoky prostřednictvím RDP (protokol vzdálené plochy) jako počátečního vektoru útoku pokračují v poklesu, ale stále zůstávají druhým nejdůležitějším vektorem pro ransomware útoky.
Útočníci se snaží prolomit slabá RDP přihlašovací jména a hesla, zejména pokud není povolena MFA (vícefaktorová autentizace).
V současné době se phishing stal nejvíce používaným útočným vektorem pro získání počátečního základu v organizaci.
Obě tyto metody jsou levné a proto nejvýnosnější pro útočníky.

Klasické operace ransomware sbírají informace před dalšími kroky, jako je vydírání s nebo bez šifrování souborů.
Pokud společnost odmítne platit, informace jsou zveřejněny a/nebo data jsou zveřejněna na tzv. leak sitech.
Tyto leak sitey seskupují všechny oběti konkrétního RaaS útočníka a často jsou dostupné pouze prostřednictvím Tor.

Nové trendy:
Obětí bylo nabídnuto, aby si "koupila" svá data dříve, než je koupí konkurent.
Tento hrozný aktér aktivně znehodnocoval webové stránky svých obětí tím, že na ně zobrazoval výkupné poznámky.
Toto je měnící se taktika, protože během počáteční fáze klasické infekce ransomware v určitém časovém okně byl útok nepředstaven, aby mohla být vedena jednání a přesvědčit společnost, aby zaplatila výkupné.
Často se stává, že společnost končí na stránkách s úniky dat pouze tehdy, když odmítne platit nebo spolupracovat.
Tento nárůst veřejného hanobení má přímý vliv na vztahy s veřejností a reputaci společnosti.
...
~\cntcapfigure{ENISA_Ransomware}{\linewidth}{Počet zaznamenaných Ransomware incidentů a velikost ukradených datve sledovaném období květen 2021 až červen 2022}{\cite{Enisa_thread_landscape}}\label{fig:ENISA_Ransomware}
...
Creating a public search site has become a new way for threat actors to get third-party victims to identify themselves quickly.
If these affected customers contact the RaaS actor
...
importance of proper patch management and a threat-informed approach to the risk management of vulnerabilities

Some american states prohibited agencies/public entities from
paying ransoms.
In addition, the Cyber incident Reporting Act obliges
...
%todo add taktiky z mitre enterprise, ics?
...
\subsubsection{Scénář 2 - doplnit název}
