...
V rámci většiny typů útoků útočník není ovlivněn lokací, jeho počáteční náklady byvají minimální a přesto výnosnost těchto útoků je značná.
...
%todo
Dnes žijeme v propojené společnosti, kde cloud, edge a IoT technologie a aplikace produkují obrovské množství dat každou sekundu. Tato data jsou základní pro všechny podniky, které chtějí konkurovat na globálním trhu, a musí být správně spravována a analyzována. Lepší správa a analýza dat vede k rychlejším procesům, lepšímu řízení zákaznických vztahů a nižším provozním nákladům. Navíc strojové učení (ML) a umělá inteligence (AI) jsou stále více přijímány a podporují migraci od tradičních softwarových systémů založených na deterministických algoritmech k systémům, kde ML nebo AI modely používají rozumování na základě dat k výpočtu řešení pro jednotlivé instance problému. Tato migrace přináší novou vlnu rizik, která tlačí směrem k 'AI Act', navrhované evropské zákonodární úpravě o umělé inteligenci (AI) - prvnímu zákonu o AI od hlavního regulátora kdekoliv.

Hrozby proti datům představují sbírku hrozeb, které cílí na zdroje dat s cílem získat neoprávněný přístup a zveřejnění, stejně jako manipulaci s daty, aby se zasáhlo do chování systému. Tyto hrozby jsou také základem mnoha existujících hrozeb, které jsou také diskutovány v tomto zprávě. Například ransomware, RDoS (útoky na zdroje), DDoS (distribuované útoky odmítnutí služby) mají za cíl odepřít přístup k datům a možná vyžadovat platbu za obnovení tohoto přístupu. Dezinformace a šíření nepravdivých informací vycházejí z manipulace s daty. Phishing, včetně nových implementací založených na deepfakes, se také zakládá na manipulaci s daty.
Porušení bezpečnosti dat je definováno v GDPR (Nařízení o ochraně osobních údajů) jako jakékoliv porušení bezpečnosti vedoucí k náhodnému nebo nezákonnému zničení, ztrátě, změně nebo neoprávněnému zveřejnění nebo přístupu k osobním údajům přenášeným, uloženým nebo jinak zpracovávaným (článek 4.12 GDPR). Technicky řečeno jsou hrozby proti datům hlavně klasifikovány jako porušení bezpečnosti dat a únik dat. Ačkoli jsou často používány jako zaměnitelné pojmy, zahrnují zásadně odlišné koncepty, které se liší především v tom, jak se stávají436 437.

Porušení bezpečnosti dat je úmyslný útok provedený kyberzločincem s cílem získat neoprávněný přístup a uvolnění citlivých, důvěrných nebo chráněných dat. Jinými slovy, porušení bezpečnosti dat je úmyslný a silový útok proti systému nebo organizaci s cílem krádeže dat.
Únik dat je událost, která může způsobit neúmyslné uvolnění citlivých, důvěrných nebo chráněných dat z důvodu například nesprávné konfigurace, zranitelností nebo lidských chyb. Nezahrnuje úmyslné útoky.
Zjednodušeně řečeno, porušení bezpečnosti dat může nastat v důsledku kyberútoku, zatímco únik dat je neúmyslná ztráta nebo vystavení dat. Porušení bezpečnosti dat je nejtrvalejší hrozbou pro data. Nicméně exponenciální růst propojených systémů a digitální transformace zvýšily význam úniků dat kvůli rostoucímu rozšiřování plochy útoku a rostoucímu zapojení uživatelů do fungování softwarových systémů.

Kromě úniku dat a porušení bezpečnosti dat se rostoucí používání modelů strojového učení nebo umělé inteligence v jádru nových distribuovaných systémů a rozhodování stává středem pozornosti. Otrava dat a útoky ze strany protivníků se stávají široce rozšířenými s cílem podkopat důvěru v IT a výrobní systémy a obecněji ve společnost jako celek. Na jedné straně se útoky na manipulaci s daty zaměřují na moderní systémy ovlivňující přesnost jejich výsledků; na druhé straně se útoky na manipulaci s daty zaměřují na lidi šířící dezinformace.

Verizon zaznamenává, že zhruba 80\% celkového počtu kompromitovaných dat pochází z vnějších zdrojů mimo cílovou organizaci, zatímco zhruba 20\% pochází z vnitřních zdrojů. Motivací je stále převážně finanční zisk (asi 90\%) a dále špionáž (méně než 10%).

Ve skutečnosti je 82\% porušení bezpečnosti dat spojeno s lidským prvkem; to lze snadno vysvětlit tím, že sociální inženýrství a různé chyby (nesprávné doručení a nesprávná konfigurace) jsou mezi hlavními vzory útoků, na třetím a čtvrtém místě, za systémovým proniknutím (které také zahrnuje sociální útoky) a základními útoky na webové aplikace.

Navzdory důležitosti lidského faktoru při porušení bezpečnosti dat je důležité poznamenat, že zneužití zranitelností jako příčiny porušení bezpečnosti dat se zdvojnásobilo a dosáhlo 7\% z celkového počtu případů letos.

-přidat zmínku o již zmíněném ransomware, to sem taky patří
%todo dopln
...
Threats against data form a collection of threats that target data sources with the aim of gaining
...
expansion of the attack surface as well as the increasing involvement of users in the workings of software systems
...
other side, data manipulation attacks target people disseminating disinformation


Verizon observes that around 80\% of total data compromises comes from outside
the target organisation, while around 20\% comes from inside. The motivation is still mainly financial gain (around
90\%) and, second by far, espionage (less than 10\%)

In fact, 82\% of data breaches
...
outperformed by system intrusion (which also involves social attacks) and basic web application attacks.
...
Krádež identity, kde zákeřní aktéři používají osobně identifikovatelné informace (PII) k tomu, aby se vydávali za uživatele, je stále častějším problémem. Navíc, vzhledem k významu dat, zejména soukromých a citlivých dat, se protivníci kombinují stále sofistikovanějšími hrozbami k cílení na data, jako jsou ransomware útoky nebo útoky na dodavatelský řetězec, stejně jako distribuované odmítání služby (DDoS) a dezinformace. Je zajímavé, že podle zprávy Evropské agentury pro kybernetickou bezpečnost (ENISA) o hrozbách v oblasti dodavatelského řetězce v roce 2021 bylo pro zhruba 58 % analyzovaných incidentů v dodavatelském řetězci cílem především zákaznických dat, včetně osobně identifikovatelných informací (PII) a duševního vlastnictví.

V důsledku nárůstu úniků dat jsou osobní a citlivá data snadno dostupná zákeřným aktérům prostřednictvím online fór a temného webu. To mělo kaskádový efekt na krádeže identity. Podle americké Federální obchodní komise (FTC) bylo v roce 2021 přijato 1,4 milionu hlášení o krádeži identity a nejvíce zasažení jsou oběti ve věku 30 až 39 let. Podle společnosti McAfee je zneužití platebních karet nejběžnějším typem krádeže identity.
...
Důvěryhodná data jsou ve skutečnosti předpokladem pro implementaci bezpečných autonomních a adaptivních systémů postavených na datech. Zejména centrální role sbíraných dat a odpovídajících závěrů o chování moderních systémů zvyšuje riziko zavádění datových znečištění a manipulace. Tyto hrozby se stávají základními hrozbami pro datově řízené systémy, kde není třeba chránit a zaručit pouze integritu dat, ale také jejich původ, nezadatelnost a odpovědnost.

V tomto kontextu se šíří ransomwarové útoky (viz kapitola 3) a deepfake (viz kapitola 9), které cílí na integritu a dostupnost dat, a tím zavádějí značné riziko rozhodování založeného výhradně na neověřených datech. Například podvodný bankovní převod ve výši téměř 35 milionů dolarů byl proveden prostřednictvím deepfake hlasového hovoru.

Přímým důsledkem datového znečištění a manipulace je snížení přesnosti modelů strojového učení.
...
the integrity and availability of data, introducing substantial risks to decisions built entirely on unverified data.
For instance, a deepfake voice call resulted in a fraudulent bank transfer of nearly \$35 million

A direct consequence of data poisoning and manipulation is a decrease in the accuracy of machine learning models.

\subsubsection{Sql injection} %todo reevaluate if sumsection or nlparagraph
...
Distribuovaný DoS útok (DDoS) cílí na dostupnost systému a dat a i když se nejedná o novou hrozbu (v roce 2019 oslavila své 20. výročí), hraje významnou roli v hrozbách kybernetické bezpečnosti478 479. Útoky se objevují, když uživatelé systému nebo služby nemohou získat přístup k relevantním datům, službám nebo jiným zdrojům. To může být dosaženo vyčerpáním služby a jejích zdrojů nebo zahlcením komponenty síťové infrastruktury.

V tomto kontextu se ransom DoS útok (RDoS) kombinuje nebezpečí tradičního DDoS útoku, zatímco výrazně snižuje potřebu zdrojů k provedení útoku. Skupiny kyberzločinců (např. Fancy Bear, Cozy Bear, Lazarus Group a Armada Collective, kteří provádějí tyto kampaně) analyzují cílové firmy, aby našli ty s slabými a zranitelnými systémy. Poté tyto firmy vydírají tím, že jim pošlou výhružný dopis s požadavkem na výkupné, aby neútočili na jejich systém482 483. Jednoduchost RDoS útoků je základem jejich rozšíření.

Během tohoto období hlášení jsme opět pozorovali velké množství incidentů spojených s útoky typu Denial of Service (DoS). V červenci 2022 jsme zaznamenali nárůst, měsíce, který byl poznamenán mnoha útoky typu DoS. Vrchol dosáhl největší zaznamenaný útok, který byl proveden proti evropskému zákazníkovi na platformě Prolexic485, používající globálně distribuovaný útokový provoz, který dosáhl svého maxima na 853,7 Gbps a 659,6 Mbps během 14 hodin.
...
In this context, Ransom Denial of Service
...
RDoS attacks is at the basis of their adoption.
...
Hrozby DDoS se nakonec stávají pátým rozměrem války, po bojích ve vzduchu, na moři, na souši a dokonce i ve vesmíru.487 Hrozby a úrovně vydírání se zvýšily, přesouvající DDoS směrem k útokům s podporou státu. V tomto kontextu je stále častěji využíván cloud computing jako vektor hrozby pro DDoS útoky na jedné straně, a na druhé straně jako hlavní cíl útoků488.

Tradiční DDoS se přesouvá směrem k mobilním sítím a Internetu věcí (IoT). Senzory a zařízení jsou skutečně vhodným cílem DDoS útoků kvůli jejich omezeným zdrojům, které často vedou k nedostatečné ochraně zabezpečení. Zařízení je snadné poškodit, často přicházejí s chybnými konfiguracemi (např. slabými hesly)501. Současně se zvyšující se složitostí těchto mobilních systémů se stává nedostatek dovedností uživatelů v oblasti zabezpečení stále relevantnějším problémem. V tomto kontextu se DDoS snaží ohrozit dostupnost komponent, stejně jako narušit provoz jiných sítí nebo systémů, ale také může ohrozit bezpečnost uživatelů. Narůstající počet zařízení a aplikací připojených k cloudu dává protivníkům větší pole působnosti, na kterém mohou zaměřit své útoky.

Tento trend byl potvrzen také v posledním období hlášení, kdy byly DDoS útoky často spouštěny z napadených serverů nebo spotřebitelských zařízení, jako jsou produkty Internetu věcí (IoT) a širokopásmové routery502. To je často způsobeno zpožděními majitelů chytrých zařízení při aktualizaci a opravách zařízení503. Například botnet Mozi stále využívá zranitelností objevených před osmi lety a napadá neopravená zařízení, budující botnety s stovkami tisíc botů. Toto bylo potvrzeno také v experimentu v Národním institutu pro standardy a technologii (NIST) ve Spojených státech, kde výzkumníci NIST zjistili, že "admin" (uživatelsk
é jméno) a "1234" (heslo) byla nejběžnější kombinace používaná při útocích proti IoT zařízením.

%todo eng del
DDoS threats are finally becoming the fifth
...
traditional DDoS is moving towards mobile networks and IoT. Sensors and
...
most common combination used in attacks against IoT

%todo rdos
Ransomový DoS (RDoS) je novou frontou útoků typu odmítnutí služby. RDoS si klade za cíl identifikovat zranitelné systémy, které se stávají cílem útoku, a provádět různé aktivity, které vedou k finálnímu požadavku na zaplacení výkupného. RDoS může mít dvě podoby: i) útok jako první nebo ii) výhrůžka jako první. Bod i) představuje scénář, kdy je proveden útok typu DDoS a požaduje se výkupné za jeho zastavení. Bod ii) předpokládá situaci, kdy je zaslán výhrůžný dopis a důkaz ve formě menšího DoS útoku spolu s požadavkem na zaplacení výkupného. RDoS útoky jsou ještě nebezpečnější než tradiční DDoS, protože mohou být dokončeny i když útočník nemá dostatek zdrojů. RDoS je komplexní útok, který kombinuje několik přístupů a technik, jako je odmítnutí služby, ransomware, padělání identity, jen abychom jmenovali několik.

RDoS cílí na poskytovatele internetových služeb (ISP), finanční instituce a malé a střední podniky (MSP), a svůj úspěch datuje zejména na rozšíření online služeb. Kromě tradičního dvojitého výhrůžného postupu, který zahrnuje vykrádání dat obětí a hrozbu jejich zveřejnění, pozorujeme rozšíření hranic cílů, které často zahrnují klienty, obchodní partnery a zaměstnance oběti. RDoS je poté používáno k vynucení splnění požadavků na výkupné, hrozící na několika úrovních, jako je odmítnutí služby, zveřejnění důvěrných dat a zapojení spojených partnerů. Dalším krokem je migrace od jednorázového výkupného k požadavku na 1 BTC za den za "ochranu" společnosti oběti před jejich útoky.
...
identity spoofing, to name but a few

RDoS targeted ISPs, financial institutions, and small
...
QR kody jsou pro útočníky výhodné, protože člověk není schopen rozeznat pravdivý od škodlivého.
pokud tedy útočník zamíchá mezi legitimní kody své podvodné, je pro běžného člověka nemožné je odhalit.\cite{Enisa_thread_landscape, cisco_most_common_attack}
...
% todo
Jedním z vedlejších účinků investice do zdrojů je zvýšení počtu objevených 0-dnů.
Ačkoliv v mnoha případech je zveřejnění takových 0-dnů provedeno zodpovědně (s následnou komunikací se výrobcem a vydáním záplat),
samotný fakt publikace vede k dalším pokusům o zneužití ze strany oportunických hrozeb, které hledají snadný zisk.

Je velmi pravděpodobné, že v blízké budoucnosti uvidíme zvýšenou investici zdrojů do výzkumu zranitelností v těchto dodavatelských řetězcích.
Je také velmi pravděpodobné, že budeme i nadále svědky pokusů o zneužití situace ze strany oportunických hrozeb po zveřejnění zranitelností v populárních obchodních technologiích.

Výzkum zranitelností je velmi nákladný a vyžaduje mnoho investic do zdrojů, částečně kvůli zlepšené bezpečnosti těchto technologií,
ale také proto, že vždy existuje riziko, že právě objevená zranitelnost může být odhalena (a opravena) dříve, než ji útočník stihne využít.
To je jeden z důvodů, proč hrozné skupiny začínají zaměřovat přímo na bezpečnostní výzkumníky. Nakonec, proč dělat obtížné výzkumy sami,
když můžete nechat někoho jiného, aby to udělal za vás, a poté si tyto informace ukrást od nich?
A i když samotné zneužití zranitelnosti v obchodní technologii nekonstituuje útok na dodavatelský řetězec podle naší definice,
zaměřování konkrétních výzkumníků a následné použití získaných informací k získání přístupu k předem určené oběti se určitě považuje za útok na dodavatelský řetězec.

S ohledem na náklady spojené s výzkumem zranitelností ze strany hrozeb je pravděpodobné, že uvidíme zvýšené zaměření na ty jednotlivce nebo organizace,
které provádějí výzkum bezpečnostních chyb nebo zranitelností s cílem ukrást jejich poznatky
...
%todo
Dodávkový útok cílí na vztahy mezi organizacemi a jejich dodavateli. Pro tento ETL report používáme definici stanovenou v Hrozbové krajině ENISA pro dodávkové útoky, kde je útok považován za dodávkový útok, pokud se skládá z kombinace alespoň dvou útoků. Konkrétně se jedná o první útok na dodavatele, který je poté použit k útoku na cíl za účelem získání přístupu k jeho aktivům. Tento cíl může být konečný zákazník nebo jiný dodavatel. Aby byl útok klasifikován jako dodávkový útok, musí být tedy jak dodavatel, tak zákazník cíli.

Evropská komise otevřela cestu pro směrnici NIS2, která by měla řešit bezpečnost dodavatelských řetězců, představila svou kybernetickou strategii s návrhy na posílení obrany a zlepšení reakcí proti zákeřným aktivitám ovlivňujícím dodavatelský řetězec a provedla podrobný průzkum strategických oblastí pro zájmy Evropy.

Distribuce malwaru je také dosahována tzv. dodávkovými útoky. Open-source frameworky jsou buď klonovány s nakaženým malwarem, s cílem nakazit kohokoli, kdo je implementuje jako nástroje nebo balíčky ve svých projektech. Jelikož kdokoli může publikovat balíčky na open-source platformy, injekce malwaru často zůstává dlouho neodhalena.

Běžným způsobem, jak je tohoto zneužito, je tzv. "typo squatting". Tím, že je do těchto repozitářů zaveden nový balíček se jménem velmi podobným populárním balíčkům, zvyšuje se šance, že balíček bude neúmyslně odkazován vývojářem, nebo že závislost bude zavedena útočníkem na původní projekt prostřednictvím pull requestu, aniž by byl útok podezřelý. V srpnu 2021 sledovali výzkumníci zhruba 8 zákeřných python knihoven, které byly tehdy staženy více než 30 000krát.

Dalším použitím jsou zákeřné knihovny v jazyce Python, které slouží k odcizení informací, například přihlašovacích údajů. V červnu 2022 bylo zjištěno, že knihovny pygrata a loglib extrahují AWS klíče341. V srpnu 2022 bylo během výzkumu odhaleno více než 10 takových balíčků. Příkladem tohoto typu malwaru je AsciI2text, který hledá místní hesla a nahrává je zpět na infrastrukturu útočníka342.
Tyto útoky jsou nalezeny v běžných a populárních repozitářích, jako je NPM343 344, Python345 a RubyGems.

Toto bylo první varování o zvýšeném zájmu skupin hrozeb, zejména těch pocházejících z Ruska a Číny, a v menší míře Severní Koreje, o útoky na dodavatelský řetězec.

Navíc, dopad operací, zejména zaměřených na špionáž, státními aktéry prostřednictvím útoků na dodavatelský řetězec nemusí být vždy okamžitě patrný pro oběti. Ačkoliv se obecně doba trvání útočníka v síti (median dwell time)703 výrazně zkrátila, tito typy hrozeb se mohou skrývat ve stínu velmi dlouhou dobu.

Státem sponzorované skupiny nejsou jedinými aktéry v tomto prostoru. Toto bylo bolestně demonstrováno operátory ransomwaru LockBit, když kriminální gang odcizil a zveřejnil704 705 dokumenty od technologického dodavatele Abiom. Abiom dodává komunikační technologii pro nizozemskou záchrannou síť C2000, Nizozemské ministerstvo obrany, národní policii a bezpečnostní služby, mezi jinými i několik dalších vládních institucí.
...
affecting the supply chain and undertook an in-depth review660 of strategic areas for Europe’s interests.
...
This was a first warning of the increased interest of threat groups, and predominantly those
coming from Russia and China, and to a lesser extent North Korea, in supply chain attacks.
...
Log4Shell je zranitelnost typu zero-day hlášená v roce 2021 v Log4j706, rámci pro logování v Javě, která může vést k vykonání libovolného kódu. Ačkoli dle definice použité pro útoky na dodavatelský řetězec v této ETL, Log4Shell není považována za útok na dodavatelský řetězec (není zde kombinace dvou útoků), kvůli převažující přítomnosti Log4j v IT infrastruktuře707 je vhodné ji zahrnout.

V prosinci 2021 Crowdstrike odhalil operaci skupiny Aquatic Panda709, hrozby z Číny, která použila upravenou verzi Log4Shell k útoku na zranitelnou instanci VMware Horizon na velké akademické instituci. Toto nebyla jediná operace spojená s VMware Horizon, která byla zneužita. V červnu 2022 vydaly jak CISA, tak CGCYBER oznámení710 711, ve kterém hlásí o více skupinách hrozeb, které využily Log4Shell na nepřeplátovaných, veřejně dostupných serverech VMware Horizon a Unified Access Gateway.

Tito hrozboví aktéři použili Log4Shell jako počáteční přístup do prostředí svých obětí, v jednom případě do sítě pro obnovu po havárii, aby poté vytáhli citlivá data nebo umožnili vzdálený přístup do těchto cílových prostředí. Očekává se, že mnoho aktérů operujících pod státní správou bude pravděpodobně i nadále integrovat Log4Shell exploity do svého arzenálu, protože tato knihovna poskytuje cenný vstupní bod a organizace mají obtíže s identifikací, které z jejich prostředků používají zranitelnou verzi Log4Shell. Z pohledu těchto hrozbových aktérů je však Log4Shell pouze velmi výnosným vstupním vektorem mezi dlouhým seznamem dalších vektorů.
...
Log4Shell on unpatched, public-facing VMware Horizon and Unified Access Gateway servers.
...
Je popsáno testovací prostředí, instalaca vybraných nástrojů, možné problémy, které při přípravě mohou nastat a také jak tyto problémy vyřešit.
...
Výsledná práce by tedy měla stačit ktomu, aby i neznalý uživatel byl shopen nastavit testovací prostředí a otestovat základní vlastnosti svojí infrastruktury nebo zařízení.
...
Velkou výhodou je zaměření na open-source nástroje, které často vytvýří profesionálové pro své specifické potřeby.
...
předpokladem je, že útočník již zjistil minimální potřebná data ze scénáře 2, již ho nezajímý být tak skrytý ale naopak chce napadenému cíli uškodit.
...
Je tedy nutné podotknout, že pokud i volně dostupný testovací framework je chopen provést tyto operace, je na zvážení, co vše je schopnen udělat specifický software zaměřeným přímo pro tyto úkony.
