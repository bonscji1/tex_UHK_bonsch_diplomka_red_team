...
Časová lhůta existuje právě kvůli nutnému času na opravu, je však nutné aby tuto výhružku nahlašovatel dodržel, jinak přestane být efektivní.\cite{Enisa_thread_landscape}
...
Za speciální zmínku stojí útok skupiny provozující ransomwaru LockBit oproti společnosti Abiom.
...
\subsection{Rozsah testovní}\label{subsec:rozsah-testovni}
...
\subsection{Popis testované infrastruktory}\label{subsec:popis-testovane-infrastruktory}
...
Jeden z počítačů funguje jako simulační server, kde jsou ve vertuálním prostředí nasazeny 2 samostatné serverové implementace.
Jsou to \textit{Windows Server 2012 R2} a \textit{Ubuntu Server 20.04.4 Focal Fossa}.
Nasazení infrastruktury s použitím fyzických zařízení opět vyřaduje otestování správného propojení všech komponentů.
...
Jen velmi malé množství útoků probíhá bez prvotní fáze získávání informací a drtivá většina z nich je neúspěšná právě kvůli tomu, že útočnící podcenili tento první krok.
...
Zběr informací je prováděn na již zmíněném reálném cíli, podle kterého je i modelováno testovací prostředí.
