...
\paragraph{Groups}
Skupiny jsou shluky aktivit sledované pod společným názvem v bezpečnostní komunitě. Analytici sledují tyto shluky pomocí různých analytických metodologií a termínů, jako jsou hrozbové skupiny (threat groups), aktivitní skupiny (activity groups) a aktéři hrozeb (threat actors). Některé skupiny mají kvůli tomu, že různé organizace sledují podobné aktivity pod různými názvy, přiřazena více jmen spojených se podobnými aktivitami. Definice skupin organizací se mohou částečně překrývat s těmi, které jsou stanoveny jinými organizacemi, a mohou se neshodovat ohledně konkrétních aktivit.
Skupiny jsou mapovány na veřejně oznámené použití technik a jsou uvedeny původní odkazy. Poskytnuté informace nezobrazují všechny možné použití technik skupin, ale pouze podmnožinu, která je k dispozici pouze prostřednictvím otevřeného zdroje. Skupiny jsou také mapovány na hlášený software, který je používán, a na připisované kampaně, a související techniky jsou sledovány samostatně na příslušných stránkách.
\paragraph{Software}
