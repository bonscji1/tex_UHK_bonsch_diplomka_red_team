...
\section{Úvod}\label{sec:uvod}%todo do
...
\section{Úvod do problematiky kybernetické bezpečnosti}\label{sec:uvod-do-problematiky-kyberneticke-bezpecnosti}%todo move as úvod?
...
\section{Testované scénáře}\label{subsec:testovane-scenare}
...
\subsection{Scénář 2 - \textit{Discovery} a \textit{Elevate-privileges}}
...
V teoretické části práce byly předány základní znalosti o kybernetického bezpečnosti a také byl čtenář seznámen se současným stavem.
Je ukázána relevantnost této tématiky, pomocí rešerše jsou představeny nejdůležitější znalosti, příkladem jsou typy útoků nebo útočníků, a na závěr jsou pro lepší pochopení praktické části předány znalosti ohledně testování a nástrojů, jenž se k testování používají.
...
Samozřejmě, že testovací prostředí vytvořit jde, bohužel každá osoba bude jinak vnímat, zda je prostředí "jednoduché a srozumitelné".
...
Dále byly ukázány základy testování a uvedeny příklady a připomínky, které je možné aplikovat i na skutečné infrastruktury ze které testování vycházelo.
...
Výsledky testování jsou ve většině příkladů očekávány, vždy existují povrchní data, která může útočník získat, například data o uživateli, síti nebo o politice hesel bylo možné získat velmi jednoduše.
Dle očekávání také dopadly části útoků zasahující do systému hluboce, ty byly zastaveny obranou systému.
Nepříjemným zjištěním však bylo, které informace jsou považovány za povrchní a tedy jednoduše přístupné.
Například nalezení přesné verze operačního systému vede po jednoduchém vyhledávání na internetu k jeho známým zranitelnostem a dokonce i připravenému malware, které danou zranitelnost zneužívá.
...
Byly zjištěny informace, které lze využít k několika typům útoků a představují tak bezpečnostní hrozbu.

\paragraph{}
Je vhodné zmínit další možnosti výzkumu související s touto prací.
Prvním doplnění této práce závisí na úzké spolupráci s organizací, v rámci možností by bylo zajímavé vytvořit scénáře přímo podle již uskutečněných útoků na organizaci nebo podobné organizace a zjistit, zda je organizace připravena takovému útoku čelit.
...
Tato práce by bylo zkoumat několik vybraných organizací, aby bylo ukázáno na nejčastější slabá místa.
