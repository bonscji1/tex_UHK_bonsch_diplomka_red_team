...
\nlparagraph{Groups - skupiny}%todo here
...
\subsubsection{Red Canary Atomic Red}%todo here
...
\subsection{Popis testovacího prostředí}\label{subsec:popis-testovaciho-prostredi}%todo here
...
\subsubsection{Scénář 1 - recon}%todo here
...
# nmap -v -A -p- -sV -version-all -O -oN vysledek.txt "cíl_útoku"
	\end{verbatim}
	\captionsource{Ukázka možného příkazu pro Nmap}{[Vlastní]}
\end{codeblock}
...
Tento fakt a vyvolané poplašné hlášení je možné vidět na obrázcích~\ref{fig:privilege_escalation},\ref{fig:all_blocked_threats} a~\ref{fig:specific_threat}.%todo check spaces
